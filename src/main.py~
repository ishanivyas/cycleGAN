...
class CycleGAN():
    def input_setup(self):
        ...
        '''
        print('matching')
        filenames_A = tf.train.match_filenames_once("/Users/ishani/Documents/pix2pix-cycleGAN/src/imgs/*.jpg")
        #filenames_A = tf.train.match_filenames_once("*")
        print(filenames_A)
        self.queue_length_A = tf.size(filenames_A)
        filenames_B = tf.train.match_filenames_once("/Users/ishani/Documents/pix2pix-cycleGAN/src/depth-imgs/*.jpg")
    ...
    def input_read(self, sess):
        ...
        print("This is is after image_A in inputread")
        for i in range(max_images):
            image_tensor = sess.run(self.image_B)
            if(image_tensor.size == img_size*batch_size*img_layer):
                self.B_input[i] = image_tensor.reshape((batch_size,img_height, img_width, img_layer))
        print("This is is after image_b in inputread")
